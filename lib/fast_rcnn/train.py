...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters):
        ...
        # extract relevant orientations (not all boxes should be evaluated)
        ...
        # calculate L2 loss for RPN orientation (mean squared loss)
        rpn_loss_orientation = tf.reduce_mean(tf.square(tf.sub(rpn_orientation_pred_rel, rpn_orientation_targets_rel)))
        ...
        # the following value is not yet needed but could help to check results
        total_estimated_rotation = self.net.get_output('roi-data')[6] + orientation_pred
        rpn_cls_prob_reshape = self.net.get_output('rpn_cls_prob_reshape')      # not necessary

        # add up all loss terms
