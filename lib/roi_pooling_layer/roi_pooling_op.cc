...
REGISTER_OP("RoiPool")
    ...
    .Input("bottom_data: T")
    .Input("bottom_rois: T")
    .Input("bottom_orientations: T")        // Num gradients 2 generated for op name: "pool_5" do not match num inputs 3
    .Output("top_data: T")
    .Output("argmax: int32");
...
static void RoiPoolingKernel(
    OpKernelContext* context, const Tensor* bottom_data, const Tensor* bottom_rois, const Tensor* bottom_orientations,
    ...
    const int pooled_width, const TensorShape& tensor_output_shape) 
{
  ...
  ROIPoolForwardLaucher(
    ...
    width, channels, pooled_height, pooled_width, bottom_rois->flat<float>().data(), bottom_orientations->flat<float>().data(),
...
class RoiPoolOp<Eigen::GpuDevice, T> : public OpKernel {
 public:
  ...
  {
    // Grab the input tensor
    const Tensor& bottom_data = context->input(0);  // VGG output
    const Tensor& bottom_rois = context->input(1);  // rois,  ROIs (0, x1, y1, x2, y2)?
    const Tensor& bottom_orientations = context->input(2);  // rois (with orientation ?)
    ...
    // orientations should have 2 dimensions.
    OP_REQUIRES(context, bottom_orientations.dims() == 2,
                errors::InvalidArgument("orientations must be 2-dimensional"));
    ...
    // print some information - not needed
    /*std::cout << "bottom_rois.dim_size(0): " << bottom_rois.dim_size(0) <<  "\n";       // 128
    std::cout << "bottom_rois.dim_size(1): " << bottom_rois.dim_size(1) <<  "\n";       // 5
    std::cout << "bottom_orientations.dim_size(0): " << bottom_orientations.dim_size(0) <<  "\n";   // 128
    std::cout << "bottom_orientations.dim_size(1): " << bottom_orientations.dim_size(1) <<  "\n";   // 1*/
    ...
    RoiPoolingKernel(context, &bottom_data, &bottom_rois, &bottom_orientations, spatial_scale_, num_rois, data_height,
...
static void RoiPoolingGradKernel(
    OpKernelContext* context, const Tensor* bottom_data, const Tensor* bottom_rois, const Tensor* bottom_orientations, const Tensor* argmax_data, const Tensor* out_backprop,
...
class RoiPoolGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    const Tensor& bottom_orientations = context->input(4);

    /*std::cout << "out_backprop.dim_size(0): " << out_backprop.dim_size(0) <<  "\n";
    std::cout << "out_backprop.dim_size(1): " << out_backprop.dim_size(1) <<  "\n";
    std::cout << "out_backprop.dim_size(2): " << out_backprop.dim_size(2) <<  "\n";
    std::cout << "out_backprop.dim_size(3): " << out_backprop.dim_size(3) <<  "\n";*/
    ...
    RoiPoolingGradKernel(
      context, &bottom_data, &bottom_rois, &bottom_orientations, &argmax_data, &out_backprop,
