...
class Network(object):
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        # inputs: 'rpn_cls_prob_reshape', 'rpn_bbox_pred', 'im_info', 'rpn_orientation_pred'
        if isinstance(input[0], tuple):
            input[0] = input[0][0]
        blob_with_orientation = tf.py_func(proposal_layer_py,
                              [input[0], input[1], input[2], input[3], cfg_key, _feat_stride, anchor_scales],
                              [tf.float32])
        blob_wo = tf.reshape(blob_with_orientation, [-1, 6], name=name)
        #blob = blob_wo[:, 0:5]     # how the original blob looked like -> without rotation
        """blob_new = tf.convert_to_tensor(blob, name='rpn_rois')
        orientation = tf.reshape(output_orientation, [-1, 1])
        orientation_new = tf.convert_to_tensor(blob, name='rpn_orientation')"""
        return blob_wo
